...
class GridWorld(tk.Tk, object):
    def __init__(self, grid_world_h=5, grid_world_w=5):
        ...
        self.geometry('{0}x{1}'.format((grid_world_h + 1) * UNIT, (grid_world_w + 1) * UNIT))
    ...
    def _build_grid_world(self):
        ...
        # well (2, 2)
        wall_center = origin + np.array([UNIT * 2, UNIT * 2])
        self.wall_1 = self.canvas.create_rectangle(
            wall_center[0] - 15, wall_center[1] - 15,
            wall_center[0] + 15, wall_center[1] + 15,
            fill='black')
        # well (2, 3)
        wall_center = origin + np.array([UNIT * 2, UNIT * 3])
        self.wall_2 = self.canvas.create_rectangle(
            wall_center[0] - 15, wall_center[1] - 15,
            wall_center[0] + 15, wall_center[1] + 15,
            fill='black')

        # end point (3, 3)
        target_center = origin + UNIT * 3
    ...
    def step(self, action):
        ...
        if next_state_coords == self.canvas.coords(self.target):
            ...
            next_state_index = 'terminal'
        # touch the wall
        elif next_state_coords in [self.canvas.coords(self.wall_1), self.canvas.coords(self.wall_2)]:
            reward = -1
            done = False
            next_state_coords = s
